    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel2">请输入添加权限的信息</h4>
            </div>
            <div class="modal-body">
                <form id="aform">
                    <div class="form-group">
                        权限名称：<input type="text" class="form-control" name="permission_name"/>
                    </div>
                    <div class="form-group">
                        权限路径：<input type="text" class="form-control" name="permissionurl"/>
                    </div>
                </form>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="myform">
                    <div class="form-group">
                        权限ID：<input type="text" class="form-control" id="pid" name="pid" readonly/>
                        权限名称：<input type="text" class="form-control" name="permission_name"/>
                        权限路径：<input type="text" class="form-control" name="permissionurl"/>
                    </div>
                </form>
...
<script>
    ...
    var Tableinit = function() {
        ...
        oTableinit.init = function() {
            $("#mytable").bootstrapTable({
                //请求后台的URL
                url: '${pageContext.request.contextPath}/selectAllPermissionByPage',
                method : 'post',//请求方式
    ...
    function edit(id) {
        $.ajax({
            url : "${pageContext.request.contextPath}/selectPermissionById",
            data : {
                "pid"    : id
            },
            async :false,
            dataType:"json",
            success : function(p) {
                $("input[name=pid]").val(p.pid);
                $("input[name=permission_name]").val(p.permission_name);
                $("input[name=permissionurl]").val(p.permissionurl);
            }
        })
    }
    ...
    function doEidt(){
        $.ajax({
            url : "${pageContext.request.contextPath}/updatePermission",
    ...
    function add(){
        $.ajax({
            url : "${pageContext.request.contextPath}/addPermission",
    ...
    function del(id) {
        $.ajax({
            url : "${pageContext.request.contextPath}/deletePermissionById",
            data : {
                "pid" : id
            },
            method : 'post',//请求方式
